...
def parse_kraken(kraken_file):
    ...
    with open(kraken_file) as f:
        ...
        for line in f:
            ...
            if line[0] == 'C': #if first character is 'C', then this is a comment line and can be skipped.  

                continue

            else: #otherwise, this is a data line and should be parsed.  

                row = re.split('\t|\n',line) #split the line into its components using tab or newline as delimiter.  

                sample_id = row[1] #second column contains sample id (first column contains read id).  

                taxon = row[4] #fifth column contains taxon name (fourth column contains read length).  

                if sample_id not in dic_kraken: #if this is the first time seeing this sample id...  

                    dic_kraken[sample_id] = {} #create a new dictionary entry for this sample id...  

                    dic_kraken[sample_id][taxon] = 1 #and initialize count for this taxon to 1 (since we've seen it once).  

                else:  #otherwise, we've already seen this sample id before...  

                    if taxon not in dic_kraken[sample_id]:  #if we haven't seen this particular taxon before...  

                        dic_kraken[sample_id][taxon] = 1  #initialize count for that particular taxon to 1 (since we've seen it once).
                    else:  #otherwise, we've already seen both the sample id and the particular taxon before...
                        dic_kraken[sample_id][taxon] += 1  ##increment count by one since we've just seen that particular combination again.
...
def makeTableFromKrakOutAndTaxDict(dKrakOut,dTaxDict):
    ...
    cols=[x+';'+dTaxDict[x]+';'+'unclassified'+';'+'unclassified;'+'unclassified;'+'unclassified;'+'unclassified;'+'unclassified;unknown;unknown;unknown;unknown;unknown;unknown;unknown;unknown;unknown;unknown;;unknown;;unknown;;unknown;;unknown;;unknown;;unknown;;unknown;;unknown;;unknown;;Unknown;;;;Unknown;;;;Unknown;;;;Unknown;;;;Unknown;;;;Unknown;;;;Unknown;;;;Unknown;;;;Unknown;;;;Unknown;;;;Unknown;;;;Unassigned;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;Unassigned;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;Unassigned;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;']
    for i in range(len(rows)):            
        ...
        cols=[x+';'+(dTaxDict[x]) +';unclassified ; unclassified ; unclassified ; unclassified ; unclassified ; unknown ; unknown ; unknown ; unknown ; unknown ; unknown ; unknown ; unknown ; unknown ;; unknown ;; unknown ;; unknown ;; unknown ;; unknown ;; unkn']
