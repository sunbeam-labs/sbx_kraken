...
def write_kraken2_tsv_summary(
    reports: list[Iterator[dict[str, float | int | str]]],
    ...
    while any([r for _, r in next_lines.items()]):
        ...
        for n, r in next_lines.items():
            if r:
                if (
                    len(r["scientific_name"]) - len(r["scientific_name"].lstrip())
                    < lowest_rank
                ):
                    lowest_rank = len(r["scientific_name"]) - len(
                        r["scientific_name"].lstrip(" ")
                    )
...
write_kraken2_tsv_summary(
    [parse_kraken2_tsv_report(open(x)) for x in snakemake.input.reports],
    report_names,
    open(snakemake.output.summary, "w"),
)
